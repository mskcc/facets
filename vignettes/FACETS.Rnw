...
polymorphic positions). C++ code ({\tt snp-pileup}) to generate such read count
files and instructions for using it are available in {\tt extcode} directory.
This program generates csv file in which the first 4 columns are: Chromosome,
Position, Ref and Alt. Then for each bam file 4 columns with names File\#R,
File\#A, File\#E and File\#D giving the counts of number of reads with the ref
allele, alt allele, errors (neither ref nor alt) and deletions in that
position. The hashmark (\#) is the number in position of the file in the
sequence of bam files given to the pileup code. In the included data the ref
and alt alleles were replaced with ``N'' and random noise added to the SNP
positions to deidentify the case. \\
...
{\sc Note:} Instructions for using the {\tt snp-pileup} code to generate the
read count matrix are given in the included {\tt README.txt} file. Since the
read counts are expected to be in the normal-tumor order we invoke the command
as {\tt snp-pileup -g -q15 -Q20 -P100 -r25,0  vcffile outputfile normalbam tumorbam}
...
over GC content along 1kb windows along the genome.
...
minor copy=1), we report cellular fraction as 1 (100\% normal).
