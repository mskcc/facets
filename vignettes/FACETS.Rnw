...
This document presents an overview of the {\tt FACETS} package. This package is
...
polymorphic positions). C++ code to generate such read count files and
instructions for using it are available in {\tt extcode} directory. This
program generates csv file in which the first 4 columns are: Chormosome,
...
and alt allele information were removed and random noise added to the SNP
...
than a pre-determined critical value (cval), we decalre a change exists and the
