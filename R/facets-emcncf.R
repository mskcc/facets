#last update:04/20/15
#input
#jointseg: generated by facets.seg
#out: segment output by facets.seg
#parameters
#mu: logR normal mean
#k: allelic ratio
...
# source("/home/nfs/shenr/LUADdriverclonality/TCGA/findmode.R")

emcncf=function(jointseg,out,trace=F,unif=F,dipLogR=NA,dipt=NA,maxiter=10,eps=1e-4){  
  ...
  #calculate LogR (logR for diploid heterozygous segments) based on allelic ratio (mafR) balanced region
  lrdata = logR[rep(mafR<0.06, nmark)]  
  dipLogR0=0
  if(sum(!is.na(lrdata))>2){  
  den=density(na.omit(lrdata))
  dipLogR0=den$x[which.max(den$y)]
  if(is.na(dipLogR0)){dipLogR0=0}
  }
  ...
  subseg1=seg[which(seg$chr<23&seglen>40),] #get big autosome segments
  seglogr1=subseg1$cnlr.median
  mafR1=subseg1$mafR
  mafR1[mafR1<0]=0
    
  cncf <- clusteredcncf(seg, dipLogR = dipLogR0)
  ...
  if(max(seg$mafR[seglen>10],na.rm=T)<0.1){
    ...
    rho=NA
    gamma=2
    out1=data.frame(cncf,cf.em=rhov.em,tcn.em=major.em+minor.em, lcn.em=minor.em)
    out1$tcn.em[is.na(out1$tcn.em)] <- out1$tcn[is.na(out1$tcn.em)]
    out=list(purity=rho,ploidy=gamma,cncf=out1)
    return(out)
  ...
  cf00=cncf$cf
  cf00[cncf$tcn == 2 & cncf$lcn == 1]=NA
  cf00[is.na(cncf$tcn)|is.na(cncf$lcn)]=NA
  
  rho00=mean(cf00,na.rm=T)
#   rho00=0.5    
#   if(sum(!is.na(cf00))>5){
#     rho00=find.mode(na.omit(cf00))$rho
#     rho00[rho00>1]=0.9
#   }else{
#     rho00=mean(cf00,na.rm=T)
#   }
  ...
  if(is.na(dipLogR)){
    dipt=2
    dthreshold= max(-0.2, -0.3*rho00)
    mthreshold= 1/(1-rho00) # LOH expected mafR
    if(any(seglogr1<dthreshold&mafR1<0.02,na.rm=T)){
      dipt=4
      #dipLogR=median(logR[rep(mafR<0.03&seglogr<dthreshold, nmark)],na.rm=T)
      dipLogR=median(seglogr1[mafR1<0.02&seglogr1<dthreshold],na.rm=T)
    }else{    
    #if any loss exceeds the maximum expected value of mafR (log(1/1-rho)), has to be copy neutral LOH
    if(any(seglogr1<dthreshold&mafR1>mthreshold,na.rm=T)){
      dipt=4
      dipLogR=min(seglogr1[seglogr1<dthreshold&mafR1>mthreshold])
    }else{      
    #large scale allelic imbalance indicate genome-doubling
    if(sum(nmark[mafR<0.06],na.rm=T)/sum(nmark)<0.5){
      dipt=4
      dipLogR=min(seglogr1[seglogr1<dthreshold])
          }    
        }   
      }
    }
  if(is.na(dipLogR)){dipLogR=dipLogR0}
  
  
    
#     else{    
#       if(any(seglogr1<dthreshold&mafR1>1,na.rm=T)){
#         if(abs(diff(range(seglogr1[mafR1>1&seglogr1<dthreshold],na.rm=T)))>0.2){
#           dipt=4
#           dipLogR=min(logR[rep(mafR>1&seglogr<dthreshold, nmark)],na.rm=T)
#         }
#       }
#     }
  

  #get initial rho from naive gcf 
  cncf <- clusteredcncf(seg, dipLogR = dipLogR)
  ...
  genotype.lsd=paste("A",major.lsd,"B",minor.lsd,sep="")
  rhov.lsd[major.lsd==1&minor.lsd==1]=NA
  rhov.lsd[is.na(major.lsd)|is.na(minor.lsd)]=NA
  rhov.lsd[rhov.lsd<0.1]=NA
  #sample purity set as the mode closest to 1
  if(sum(!is.na(rhov.lsd)>5)){
    rho=find.mode(na.omit(rhov.lsd))$rho
    rho[rho>1]=0.9
  } else {
    rho = mean(rhov.lsd,na.rm=T)
  }
  #rho=max(rho,0.5,na.rm=T) #avoid small initial value of purity 
  
  lowpur=F  
  if(mean(rhov.lsd[rhov.lsd>0.2&seglen>10],na.rm=T)<0.35){lowpur=T}
  ...
  rhov=as.vector(by(rhov,cncf$segclust,mean))
  }
  ...
  while(dif>eps && iter<maxiter) {
    ...
    #den=density(na.omit(rep(rhov[which.geno!=4 & !is.na(which.geno)],nmark[which.geno!=4 & !is.na(which.geno)])))
  ...
  out1=data.frame(cncf,cf.em=rhov.em,tcn.em=major.em+minor.em, lcn.em=minor.em)
  out1$tcn.em[is.na(out1$tcn.em)] <- out1$tcn[is.na(out1$tcn.em)]
  out=list(loglik=loglik,purity=rho,ploidy=gamma,dipLogR=dipLogR,dipt=dipt, cncf=out1)
...
find.mode=function (x) 
{
  ...
  modes = y[modes.pos]
  idx = order(modes, decreasing = T)[1:2]
  signodes = modes.pos[modes > 0.5]
  if (length(signodes) >= 2) {
    rho = max(den$x[modes.pos[idx]])
  }
  out = list(rho = rho, nmodes = nmodes)
  out
}

plotSampleCNCF=function(jointseg,out,fit) {
  mat=jointseg
  ...
  plot(mat$cnlr, pch=".", axes=F,cex=1.5,ylim=c(-3,3), col = c("grey","lightblue")[1+rep(cncf$chr-2*floor(cncf$chr/2), cncf$num.mark)], ylab="log-ratio")
  ...
  plot(mat$valor, axes=F, pch=".", cex=1.5, col = c("grey","lightblue")[1+rep(cncf$chr-2*floor(cncf$chr/2), cncf$num.mark)], ylab="log-odds-ratio", ylim=c(-4,4))
